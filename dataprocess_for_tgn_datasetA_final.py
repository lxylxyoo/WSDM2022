...
if __name__ == "__main__":
    input_A_initial = pd.read_csv("data/final_test_csvs/input_A.csv", header=None)
    input_A_initial.columns = ['src', 'tgt', 'edge_type', 'ts_start', 'ts_end']
    ...
    input_A_initial6_s = pd.concat([input_A_initial5.iloc[:,:4], pd.Series([1.0] * input_A_initial5.shape[0]), input_A_initial5.iloc[:,4:]], axis=1).drop(['ts_end'], axis='columns')
    input_A_initial6_e = pd.concat([input_A_initial5.iloc[:,:4], pd.Series([1.0] * input_A_initial5.shape[0]), input_A_initial5.iloc[:,4:]], axis=1).drop(['ts_start'], axis='columns')
    ...
    input_A_initial6_s.to_csv("data/processed/DatasetA_final_start_fill-1.csv", index=False)
    input_A_initial6_e.to_csv("data/processed/DatasetA_final_end_fill-1.csv", index=False)