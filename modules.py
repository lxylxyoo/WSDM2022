...
class MsgLinkPredictor(nn.Module):
    ...
    def __init__(self, emb_dim):
        ...
        ########################
        # self.edge_fc = nn.Linear(19, emb_dim)
        ########################
...
class MemoryModule(nn.Module):
    ...
    def reset_memory(self, device="cpu"):
        self.last_update_t = nn.Parameter(torch.zeros(
            self.n_node, device=device).float(), requires_grad=False).double()
        self.memory = nn.Parameter(torch.zeros(
            (self.n_node, self.hidden_dim), device=device).float(), requires_grad=False)
    ...
    def set_last_update_t(self, node_idxs, values):
        self.last_update_t[node_idxs] = values.to(self.last_update_t.device)
...
class MemoryOperation(nn.Module):
    ...
    def stick_feat_to_graph(self, g, feat_emb_list):
        # How can I ensure order of the node ID
        g.ndata['timestamp'] = self.memory.last_update_t[g.ndata[dgl.NID]].to(g.device)
        g.ndata['memory'] = self.memory.memory[g.ndata[dgl.NID]]
        #############
        # Dataset A feature embedding
        if feat_emb_list is not None:
            edges_feats = []
            for i, row in enumerate(g.edata['feats'].transpose(0,1)):
                edges_feats.append(feat_emb_list[i](row).reshape(-1, 100))
            edges_feats = torch.cat(edges_feats, axis=1)
            g.edata['feats_emb'] = edges_feats.to(g.device)
        #############
    ...
    def msg_fn_cat(self, edges):
        ...
        # Dataset A feature embedding
        ret = torch.cat([edges.src['memory'], edges.dst['memory'],
                         edges.data['feats_emb'], time_encode], dim=1)
        ##############
        '''
        # print('msg_fn_cat message shape', ret.shape) # []
        return {'message': ret, 
                'timestamp': edges.data['timestamp'],
                # 'label': edges.data['label'].bool()
        }
    ...
    def update_memory(self, nodes):
        ...
        return {'memory': ret, 'timestamp': nodes.data['timestamp'].double()}

    def forward(self, g, feat_emb_list):
        g = dgl.add_reverse_edges(g, copy_edata=True)
        g.remove_nodes(
                nids=((g.in_degree(v=g.nodes()) == 0).nonzero().reshape(-1))
        )
        self.stick_feat_to_graph(g, feat_emb_list)
...
class TemporalGATConv(nn.Module):
    ...
    def __init__(self,
        ...
        # Dataset A feature embedding  
        if feat_emb_A:
            cat_nums = [34,24,17,416,58,225,20,29,249,21,144,34,24,17,416,58,225,20,29]
            self.feat_emb_list = torch.nn.ModuleList()
            for cat_num in cat_nums:
                self.feat_emb_list.append(torch.nn.Embedding(cat_num, self.feat_emb_dim))
        ####################
    ...
    def weight_fn(self, edges):
        ...
        # print(edges.data['timestamp'])
        print("weigt_fn sum of edges.src['timestamp']", torch.sum(edges.src['timestamp']))
        ...
        # Dataset A feature embedding
        edges_feats = []
        for i, row in enumerate(edges.data['feats'].transpose(0,1)):
            edges_feats.append(self.feat_emb_list[i](row).reshape(-1, self.feat_emb_dim))
        edges_feats = torch.cat(edges_feats, axis=1)
        # [num_of_edges, 19 * 100]
        k = torch.cat(
            [edges.src['s'], edges_feats, time_encode], dim=1)
        #####################
        '''
